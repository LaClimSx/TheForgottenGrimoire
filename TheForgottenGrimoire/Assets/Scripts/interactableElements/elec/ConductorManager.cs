...
public class ConductorManager : MonoBehaviour
{
    ...
    {
        if (!getCircuitOfConductor(conductor2).Equals(getCircuitOfConductor(conductor1))) recomputeCircuit(conductor1, conductor2);
        printCircuitsAndSrc();
    }

    public void conductorLeavingCondictor(InteractableConductor conductor1, InteractableConductor conductor2)
    {
        if (!getCircuitOfConductor(conductor1).Equals(getCircuitOfConductor(conductor2))) recomputeCircuit(conductor1, conductor2);
        printCircuitsAndSrc();        
    }
    ...
    public void addToCircuit(InteractableConductor conductor)
    {

    }

    public void removeFromCircuit(List<InteractableConductor> circuit, InteractableConductor conductor)
    {
    ...
    private void recomputeCircuit(InteractableConductor conductor1, InteractableConductor conductor2)
    {
    ...
    private void printCircuitsAndSrc()
    {
        ...
        for (int i = 0; i < circuits.Count; i++)
        {
            ...
            msg += $"    - sources: {sources[circuits[i]]}\n";
        }
        print(msg);
    }
}
